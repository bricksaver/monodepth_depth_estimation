...
def train(params):
    ...
    with tf.Graph().as_default(), tf.device('/cpu:0'):
        ...
        print('left_image_batch: ', left)
        print('right_image_batch: ', right)
        print('POINT 8')
        # split for each gpu
        left_splits  = tf.split(left,  args.num_gpus, 0)
        right_splits = tf.split(right, args.num_gpus, 0)
        print('left_splits: ', left_splits)
        print('right_splits: ', right_splits)
        print('POINT 9')
        tower_grads  = []
        tower_losses = []
        ...
        with tf.variable_scope(tf.get_variable_scope()):
            ...
            for i in range(args.num_gpus):
                with tf.device('/gpu:%d' % i):
                    ...
                    print('left_splits[',i,']: ',left_splits[i])
                    print('right_splits[',i,']: ',right_splits[i])
...
def test(params):
    """Test function."""
    print('POINT 2')
    print('DATA_PATH: ', args.data_path, ', FILENAMES_FILE: ', args.filenames_file, ', DATASET: ', args.dataset, ', MODE: ',args.mode)
    dataloader = MonodepthDataloader(args.data_path, args.filenames_file, params, args.dataset, args.mode)
    print('POINT 2.1')
    left  = dataloader.left_image_batch
    print('POINT 2.2')
